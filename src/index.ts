...
const main = async () => {
  ...
  // const txM = await api.tx.sudo.sudo(api.tx.assets
  //   .mintInto(
  //     1,
  //     walletSudo.publicKey,
  //     api.createType("u128", (15000 * 1e12).toString())
  //   ))
  ...
  // const tx = await api.tx.tokens.transfer(palletId, 0, api.createType("u128", (15000 * 1e12).toString())).signAndSend(walletSudo, async (result1) => {
  //   console.log(`Current status is ${result1.status}`);
  //   if (result1.status.isInBlock) {
  //     console.log(`Transaction included at blockHash ${result1.status.asInBlock}`);
  //   } else if (result1.status.isFinalized) {
  //     console.log(`Transaction finalized at blockHash ${result1.status.asFinalized}`);
  //   }
  // });
