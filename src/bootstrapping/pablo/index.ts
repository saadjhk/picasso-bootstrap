...
import { toChainUnits, toConstantProductPoolInitConfig, toLiquidityBootstrappingPoolInitConfig, toPabloPoolPair, toStableSwapPoolInitConfig } from "@dev-test/utils";
import { addLiquidity, createConstantProductPool, createLiquidityBootstrappingPool, enableTwap, updateDexRoute } from "@dev-test/pallets/pablo/extrinsics";
...
export async function bootstrapPools(
    api: ApiPromise,
    wallets: KeyringPair[],
    ...
    for (const pool of config.pools as any[]) {
        ...
        try {
            if (pool.sale) {
                ...
                const lbpConfig = toLiquidityBootstrappingPoolInitConfig(api, wallets[walletIndex], pool, new BigNumber(start.toString()), 25);
            ...
            if (poolId) {
                if (pool.addLiquidity) {
                    await mintAssetsToWallets(api, [wallets[walletIndex]], walletSudo, [pool.pair.base], new BigNumber(pool.liquidityAmount.base));
                    await mintAssetsToWallets(api, [wallets[walletIndex]], walletSudo, [pool.pair.quote], new BigNumber(pool.liquidityAmount.quote));

                    const liquidityAdded = await addLiquidity(api, wallets[walletIndex], poolId, pool.liquidityAmount.base, pool.liquidityAmount.quote);
